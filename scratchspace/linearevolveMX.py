...
class Linear(object):
    ...
    def __init__(self, model, phi, rvals):
        ...
        #a = np.asarray([ self.rvals, self.delta_U ])
        #np.savetxt("deltaU0s.csv", a, delimiter=",")
    ...
    def construct_even_grid(self, gridpoints=None):
        """
        Construct the grid to perform the linear evolution on
        ...
        # We must construct the even barA grid to output our Misner-Sharp values
        # (for the sake of the nonlinear evolution finite diffs)
        # Begin by constructing the grid between 0 and 1
        self.grid_even = (np.array(list(range(gridpoints))) + 0.5) / gridpoints
        # Construct the even barA grid between 0 and max_A
        self.Agrid_even = self.grid_even * self.max_A

        # Construct the various spatial modes
        # Note that these modes are matrices. First index is the A position, second index is the wavenumber.
        # So, we can matrix multiply this by a vector of kn time values to get the desired evolutions.
        kn_times_A_even = np.outer(self.Agrid_even, self.wavenumbers)
        self.j0modes_even = spherical_jn(0, kn_times_A_even)
        self.j0modes_times_k_even = self.j0modes_even.copy() * self.wavenumbers
        self.j1modes_even = spherical_jn(1, kn_times_A_even)
        self.j1modes_on_ka_even = self.j1modes_even / kn_times_A_even
    ...
    def full_evolution(self, xi):
        ...
        #FIXME THE MODES NEED TO BE EVEN
